...
static db_test *g_head = NULL;
static db_test *g_current = NULL;  /* test in flight, for crash reporting */
...
static int by_location(const void *A, const void *B) {
  const db_test *a = *(const db_test *const *)A;
  const db_test *b = *(const db_test *const *)B;
...
int db_test_main(int argc, char **argv) {
  ...
  int n = 0;
  for (db_test *t = g_head; t; t = t->next) n++;
  db_test **arr = calloc((size_t)(n > 0 ? n : 1), sizeof *arr);
  int idx = 0;
  for (db_test *t = g_head; t; t = t->next) arr[idx++] = t;
